...
import type { DetailedAdminOrder } from '@/types/order';
...
async function getOrderDetails(orderId: string): Promise<DetailedAdminOrder | null> {
  ...
  const total = (orderWithItems as unknown as DetailedAdminOrder).items.reduce((sum: number, item: any) => {
  ...
  } as unknown as DetailedAdminOrder;
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <CardContent className="space-y-4">
                ...
                {order.items.map((item: DetailedAdminOrder['items'][0]) => (
                  <TableRow key={item.id}>
                    <TableCell className="flex items-center space-x-2">
                      <Image
                        src={item.variant.product.imagePath || '/placeholder.png'} // Provide a placeholder
            ...
            <div className="flex justify-end">
              <div className="text-right">
                <p className="text-muted-foreground">Subtotal: {formatCurrency(order.total)}</p>
                {/* Add Shipping, Taxes if applicable */}
                <p className="font-semibold text-lg">Total: {formatCurrency(order.total)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="space-y-4">
          ...
          <CardContent className="space-y-4">
            ...
            {order.billingAddress && order.shippingAddress && order.billingAddress.id !== order.shippingAddress.id && (
              <>
                <Separator />
                <div>
                  ...
                  <address className="not-italic text-sm">
                    ...
                    {order.billingAddress.city}, {order.billingAddress.state} {order.billingAddress.postal}<br />
